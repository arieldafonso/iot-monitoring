...
class TestTelemetryMessage:
    def test_from_mqtt_valid_temperature(self):
        msg = TelemetryMessage.from_mqtt("unic/rooms/room-01/telemetry/temperature", "24.5")
        assert msg.location == "room-01"
        assert msg.measurement == "temperature"
        assert msg.value == 24.5

    def test_from_mqtt_valid_humidity(self):
        msg = TelemetryMessage.from_mqtt("unic/rooms/room-01/telemetry/humidity", "65.0")
        assert msg.location == "room-01"
    ...
    def test_from_mqtt_invalid_payload_non_numeric(self):
        with pytest.raises(ValueError, match="not numeric"):
            TelemetryMessage.from_mqtt("unic/rooms/room-01/telemetry/temperature", "abc")

    def test_from_mqtt_negative_value(self):
        msg = TelemetryMessage.from_mqtt("unic/rooms/room-01/telemetry/temperature", "-5.2")
        assert msg.value == -5.2

    def test_from_mqtt_with_timestamp(self):
        ts = datetime(2024, 1, 1, 12, 0, 0)
        msg = TelemetryMessage.from_mqtt("unic/rooms/room-01/telemetry/temperature", "24.5", timestamp=ts)
    ...
    def test_repr(self):
        msg = TelemetryMessage(location="room-01", measurement="temperature", value=24.5)
        repr_str = repr(msg)
        assert "location=room-01" in repr_str
